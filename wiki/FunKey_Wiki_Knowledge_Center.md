...
To supplement the above-listed default input mapping file, a table detailing the associations between emulated system inputs and system inputs, as well as the default mapping of those inputs to the device's buttons, is listed below. Note that while the lack of X, Y, and Z mapping capabilities for the Sega Genesis and Mega Drive does not result in any game incompatibilities, the lack of Y1, Y2, Y3, and Y4 mapping capabilities for the WonderSwan and WonderSwan Color means that vertical games for that system are currently [incompatible with the FunKey S](/wiki/List_of_games_with_compatibility_issues.md#Games_that_require_improved_emulation "List of games with compatibility issues"). Following that table is another table that lists the same information for the device's shortcuts.
